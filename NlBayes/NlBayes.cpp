...
, 	unsigned tailemask){
	...
	{
	    double sum = 0;
	    for(unsigned l = 0; l < tailemask; l++)
	    {
...
,	const float Step
	...
	for(unsigned i =0;i<p_imSize.height;i++){
		...
		output[i*p_imSize.width+j]=	input[i*p_imSize.width+j]+Step*((wbR1[i*p_imSize.width+j]*(input[nx[i]*p_imSize.width+py[j]]-input[i*p_imSize.width+j]))+(wtM2[i*p_imSize.width+j]*(input[i*p_imSize.width+py[j]]-input[i*p_imSize.width+j]))+(wbL3[i*p_imSize.width+j]*(input[px[i]*p_imSize.width+py[j]]-input[i*p_imSize.width+j]))+(wmR4[i*p_imSize.width+j]*(input[nx[i]*p_imSize.width+j]-input[i*p_imSize.width+j]))+(wmL6[i*p_imSize.width+j]*(input[px[i]*p_imSize.width+j]-input[i*p_imSize.width+j]))+(wtR7[i*p_imSize.width+j]*(input[nx[i]*p_imSize.width+ny[j]]-input[i*p_imSize.width+j]))+(wmB8[i*p_imSize.width+j]*(input[i*p_imSize.width+ny[j]]-input[i*p_imSize.width+j]))+(wtL9[i*p_imSize.width+j]*(input[px[i]*p_imSize.width+ny[j]]-input[i*p_imSize.width+j])));	
...
,		const ImageSize &p_imSize
		...
		vector<float>dG3,dG3t,imgDiffXY(Im.size()),imgDiffYY(Im.size()),imgDiffXX(Im.size()),J11(Im.size()),J12(Im.size()),J22(Im.size());
		dG3.resize(3*3);dG3t.resize(3*3);
			dG3[0]=3.0/32;
			dG3[1]=0;
			dG3[2]=-3.0/32;
			dG3[3]=10.0/32;
		...
		vector<float>imgDiffX,imgDiffY;
		imgDiffX.resize(Im.size());imgDiffY.resize(Im.size());
		Filter2d(Im,imgDiffY,p_imSize,dG3,1,3);
		Filter2d(Im,imgDiffX,p_imSize,dG3t,1,3);
		...
		for(unsigned i=0;i<p_imSize.whc;i++){
			imgDiffXX[i]=imgDiffX[i]*imgDiffX[i];
			imgDiffXY[i]=imgDiffX[i]*imgDiffY[i];
			imgDiffYY[i]=imgDiffY[i]*imgDiffY[i];
		}
		/*for(unsigned i=0;i<p_imSize.width;i++){
			std::cout<<imgDiffXY[i]<<endl;
		}*/
		int sze = 9,limitXSize;
	//// Define Gaussian kernel
	std::vector<float> gaussKernel;
	getGaussianKernel(gaussKernel,sze,1,1);
	float myalpha = 0.001,sig=3;
	vector<float> mask_sig;
	Generate_mask(sig,myalpha,mask_sig,limitXSize,2);

	Filter2d(imgDiffXX,J11,p_imSize,mask_sig,limitXSize/2,limitXSize);
	Filter2d(imgDiffXY,J12,p_imSize,mask_sig,limitXSize/2,limitXSize);
	Filter2d(imgDiffYY,J22,p_imSize,mask_sig,limitXSize/2,limitXSize);



   float temp1, temp2, L1,L2, C1=0.001, lambda1, lambda2;
	   vector<float> P;
	   P.resize(2);
		for(unsigned i=0;i<p_imSize.height;i++){
			for(unsigned j=0;j<p_imSize.width;j++){
			temp1=J11[i*p_imSize.width+j]-J22[i*p_imSize.width+j];
            temp2=sqrt(temp1*temp1 +4*J12[i*p_imSize.width+j]*J12[i*p_imSize.width+j]);
            lambda1=0.5*(temp1+temp2);
            lambda2=0.5*(temp1-temp2);
			P[0]=2*J12[i*p_imSize.width+j];
			P[1]=J22[i*p_imSize.width+j] - J11[i*p_imSize.width+j]+temp2;
...
,	const ImageSize &p_imSize
	...
	spectreTensor2(im,Jxx,Jxy,Jyy,p_imSize);
...
/*
	...
	for(unsigned i=0;i<p_imSize.width;i++){
		std::cout<<v2y[i*p_imSize.width]<<endl;
	}
...
NonNegativityDiscretization(im,imFinal,p_imSize,Jxx,Jyy,Jxy,0.15);
...
,   const bool p_verbose
	...
	//! RGB to YUV
	vector<float> imNoisy = i_imNoisy;
	vector<float> imNoisy_aniso = i_imNoisy;
...
#pragma omp parallel for schedule(dynamic, nbParts/nbThreads) \
	...
	if(anisotropicSmoothing(o_imBasic,p_imSize)!= EXIT_SUCCESS){
		return EXIT_FAILURE;
	}
	//! Divide the noisy and basic images into sub-images in order to easier parallelize the process
	if (subDivide(imNoisy, imNoisySub, p_imSize, imSizeSub, paramStep2.boundary, nbParts)
        != EXIT_SUCCESS) {
		return EXIT_FAILURE;
	}
